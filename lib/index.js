...
var DemoList = class extends import_coc.BasicList {
  constructor() {
    super();
    this.name = "demo_list";
    this.description = "CocList for coc-csharp";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-csharp list item 1",
        data: { name: "list item 1" }
      },
      {
        label: "coc-csharp list item 2",
...
async function activate(context) {
  import_coc2.window.showInformationMessage("coc-csharp works!");
  context.subscriptions.push(
    import_coc2.commands.registerCommand("coc-csharp.Command", async () => {
      import_coc2.window.showInformationMessage("coc-csharp Commands works!");
    }),
    import_coc2.listManager.registerList(new DemoList()),
    import_coc2.sources.createSource({
      name: "coc-csharp completion source",
      // unique id
      doComplete: async () => {
